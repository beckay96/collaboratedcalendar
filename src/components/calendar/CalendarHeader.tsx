      ...
      <div className="flex items-center justify-between py-[3px] my-[48px]">
        <div className="flex items-center space-x-2 rounded-full bg-primary/20 px-4 py-2 cursor-pointer transition-all hover:bg-primary/30" onClick={onToggleView}>
          ...
          <h2 className="text-2xl font-semibold neon-text-purple">
            {viewType === 'month' ? getFormattedMonthYear(currentDate) : format(displayDate, 'MMMM d')}
          </h2>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button onClick={handlePrevious} className="p-2 rounded-full hover:bg-secondary/80 transition-colors duration-200">
            <ChevronLeft className="w-6 h-6" />
          </button>
          <button onClick={handleNext} className="p-2 rounded-full hover:bg-secondary/80 transition-colors duration-200">
